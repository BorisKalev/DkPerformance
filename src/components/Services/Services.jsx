      ...
      <div className="max-w-300 mx-auto">
        ...
        <header className="mb-14">
          ...
          <p className="mt-3.5 max-w-[52ch] text-muted text-base leading-[1.7]">
            From a simple tune to a full race build — we handle every part of the
            build process under one roof.
          </p>
        </header>
        ...
        <ul className="list-none grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-6">
          {SERVICES.map(({ icon, title, description }) => (
            ...
            >
              <span className="block text-[2rem] mb-4" aria-hidden="true">{icon}</span>
              <h3 className="text-[1.1rem] font-bold tracking-[0.02em] mb-2.5">{title}</h3>
